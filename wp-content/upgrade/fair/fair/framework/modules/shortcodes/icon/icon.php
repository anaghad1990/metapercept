<?php
namespace FairEdge\Modules\Shortcodes\Icon;

use FairEdge\Modules\Shortcodes\Lib\ShortcodeInterface;

/**
 * Class Icon
 * @package FairEdge\Modules\Shortcodes\Icon
 */
class Icon implements ShortcodeInterface {
            ...
            'params'                    => array_merge(
                ...
                array(
                    ...
                    array(
                        ...
                        'description' => esc_html__('Please insert border radius(Rounded corners) in px. For example: 4 ', 'fair'),
    ...
    public function render($atts, $content = null) {
		...
		if($params['icon_class'] != '') {
            $iconHolderClasses[] = $params['icon_class'];
        }
        ...
        $html = fair_edge_get_shortcode_module_template_part('templates/icon', 'icon', '', $params);
        ...
        if(($params['type'] !== 'normal' && !empty($params['shape_size'])) ||
           ($params['type'] == 'normal')
            ...
            if(!empty($params['custom_size'])) {
                $iconStyles[] = 'font-size:'.fair_edge_filter_px($params['custom_size']).'px';
            }
        }
    ...
    private function generateIconHolderStyles($params) {
        ...
        if(isset($params['type']) && $params['type'] !== 'normal') {
            ...
            if(!empty($shapeSize)) {
                $iconHolderStyles[] = 'width: '.fair_edge_filter_px($shapeSize).'px';
                $iconHolderStyles[] = 'height: '.fair_edge_filter_px($shapeSize).'px';
                $iconHolderStyles[] = 'line-height: '.fair_edge_filter_px($shapeSize).'px';
            }
            ...
            if(!empty($params['border_color']) && (isset($params['border_width']) && $params['border_width'] !== '')) {
				...
				$iconHolderStyles[] = 'border-width: '.fair_edge_filter_px($params['border_width']).'px';
            ...
            if($params['type'] == 'square') {
                if(isset($params['border_radius']) && $params['border_radius'] !== '') {
                    $iconHolderStyles[] = 'border-radius: '.fair_edge_filter_px($params['border_radius']).'px';
                }
            }
        }
