<?php
namespace FairEdge\Modules\Shortcodes\IconWithText;

use FairEdge\Modules\Shortcodes\Lib\ShortcodeInterface;

/**
 * Class IconWithText
 * @package FairEdge\Modules\Shortcodes\IconWithText
 */
class IconWithText implements ShortcodeInterface {
    ...
    public function vcMap() {
        vc_map(array(
            'name'                      => esc_html__('Icon With Text', 'fair'),
            'base'                      => $this->base,
            'icon'                      => 'icon-wpb-icon-with-text extended-custom-icon',
            'category'                  => 'by EDGE',
            'allowed_container_element' => 'vc_row',
            'params'                    => array_merge(
                fair_edge_icon_collections()->getVCParamsArray(),
                array(
                    array(
                        'type'       => 'attach_image',
                        'heading'    => 'Custom Icon',
                    ...
                    array(
                        ...
                        'group'      => 'Icon Settings',
						'dependency' => array('element' => 'icon_gradient', 'value' => 'no')
                    ),
                    array(
                        'type'       => 'colorpicker',
    ...
    public function render($atts, $content = null) {
        ...
        return fair_edge_get_shortcode_module_template_part('templates/iwt', 'icon-with-text', $params['icon_position'], $params);
    }
    ...
    private function getIconParameters($params) {
        ...
        if(empty($params['custom_icon'])) {
            ...
            if($params['icon_gradient'] == 'yes') {
                $params_array['icon_class'] = 'edgtf-type1-gradient-left-to-right-text';
            }
    ...
    private function getContentStyles($params) {
        ...
        if($params['icon_position'] == 'left' && !empty($params['text_left_padding'])) {
            $styles[] = 'padding-left: '.fair_edge_filter_px($params['text_left_padding']).'px';
        }

        if($params['icon_position'] == 'right' && !empty($params['text_right_padding'])) {
            $styles[] = 'padding-right: '.fair_edge_filter_px($params['text_right_padding']).'px';
        }
