...
class EdgeSkin extends FairEdgeSkinAbstract {
    /**
     * Skin constructor. Hooks to fair_edge_admin_scripts_init and fair_edge_enqueue_admin_styles
     */
    public function __construct() {
        $this->skinName = 'edge';

        //hook to
        add_action('fair_edge_admin_scripts_init', array($this, 'registerStyles'));
        add_action('fair_edge_admin_scripts_init', array($this, 'registerScripts'));

        add_action('fair_edge_enqueue_admin_styles', array($this, 'enqueueStyles'));
        add_action('fair_edge_enqueue_admin_scripts', array($this, 'enqueueScripts'));

        add_action('fair_edge_enqueue_meta_box_styles', array($this, 'enqueueStyles'));
        add_action('fair_edge_enqueue_meta_box_scripts', array($this, 'enqueueScripts'));
    ...
    public function registerScripts() {
        ...
        foreach ($this->scripts as $scriptHandle => $scriptPath) {
            fair_edge_register_skin_script($scriptHandle, $scriptPath);
        }
    }
    ...
    public function registerStyles() {
        ...
        foreach ($this->styles as $styleHandle => $stylePath) {
            fair_edge_register_skin_style($styleHandle, $stylePath);
        }
    ...
    public function renderOptions() {
        ...
        $tab    = fair_edge_get_admin_tab();
        $active_page = $fair_edge_Framework->edgtOptions->getAdminPageFromSlug($tab);
     ...
     *
*@param FairEdgeAdminPage $page current page to load
     *
     * @see EdgeSkinAbstract::loadTemplatePart()
     */
