...
if( ! class_exists('FairEdgeSidebar') ){
	
	class FairEdgeSidebar{
		...
		function __construct(){
			...
			$this->title = esc_html__('Custom Widget Area','fair');
		...
		function template_add_widget_field(){
			...
			echo "\n    <span class='input_wrap'><input type='text' value='' placeholder = '".esc_html__('Enter Name of the new Widget Area', 'fair')."' name='edgt-add-widget' /></span>";
			echo "\n    <input class='button' type='submit' value='".esc_html__('Add Widget Area', 'fair')."' />";
			echo "\n    ".$nonce;
			echo "\n  </form>";
			echo "\n</script>\n";
		}
		...
		function register_custom_sidebars(){
			...
			$args = apply_filters('fair_edge_custom_widget_args', $args);
