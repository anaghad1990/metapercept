...
class FairEdgeImport {

    public $message = "";
    public $attachments = false;
    function FairEdgeImport() {
        add_action('admin_menu', array(&$this, 'edgt_admin_import'), 99);
    ...
    public function import_content($file){
        if (!class_exists('WP_Importer')) {
            ...
            $this->message = esc_html__("Error loading files", "edge-cpt");
        }
    }
    ...
    public function import_options($file){
        $options = $this->file_options($file);
        update_option( 'edgt_options_fair', $options);
    ...
    function edgt_admin_import() {
        if (edgt_core_theme_installed()) {
            ...
            $this->pagehook = add_submenu_page(
                ...
                array($fair_edge_Framework->getSkin(), 'renderImport')
            );

            add_action('admin_print_scripts-'.$this->pagehook, 'fair_edge_enqueue_admin_scripts');
            add_action('admin_print_styles-'.$this->pagehook, 'fair_edge_enqueue_admin_styles');
...
function edgt_init_import_object(){
    global $fair_edge_import_object;
    $fair_edge_import_object = new FairEdgeImport();
}

add_action('init', 'edgt_init_import_object');

if(!function_exists('fair_edge_dataImport')){
    function fair_edge_dataImport(){
        global $fair_edge_import_object;

        if ($_POST['import_attachments'] == 1)
            $fair_edge_import_object->attachments = true;
        else
            $fair_edge_import_object->attachments = false;

        $folder = "fair/";
        if (!empty($_POST['example']))
            $folder = $_POST['example']."/";

        $fair_edge_import_object->import_content($folder.$_POST['xml']);

        die();
    }

    add_action('wp_ajax_edgt_dataImport', 'fair_edge_dataImport');
}

if(!function_exists('fair_edge_widgetsImport')){
    function fair_edge_widgetsImport(){
        global $fair_edge_import_object;
        ...
        $fair_edge_import_object->import_widgets($folder.'widgets.txt',$folder.'custom_sidebars.txt');

        die();
    }

    add_action('wp_ajax_edgt_widgetsImport', 'fair_edge_widgetsImport');
}

if(!function_exists('fair_edge_optionsImport')){
    function fair_edge_optionsImport(){
        global $fair_edge_import_object;
        ...
        $fair_edge_import_object->import_options($folder.'options.txt');

        die();
    }

    add_action('wp_ajax_edgt_optionsImport', 'fair_edge_optionsImport');
}

if(!function_exists('fair_edge_otherImport')){
    function fair_edge_otherImport(){
        global $fair_edge_import_object;
        ...
        $fair_edge_import_object->import_options($folder.'options.txt');
        $fair_edge_import_object->import_widgets($folder.'widgets.txt',$folder.'custom_sidebars.txt');
        $fair_edge_import_object->import_menus($folder.'menus.txt');
        $fair_edge_import_object->import_settings_pages($folder.'settingpages.txt');

        die();
    }

    add_action('wp_ajax_edgt_otherImport', 'fair_edge_otherImport');
}