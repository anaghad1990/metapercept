...
if( ! empty($row_type) && $row_type == 'parallax'){
	...
	if(fair_edge_options()->getOptionValue('parallax_on_off') == 'off'){
...
if($row_type == 'row' && $css_animation != ''){
	...
	if($transition_delay !== ''){
		...
		$inner_start .= '<div '.fair_edge_get_inline_style($animation_styles).'>';
		$inner_end .= '</div>';
	}else{
		$inner_start .= '<div>';
		$inner_end .= '</div>';
...
if($rounded_tab == 'yes'){
	...
	$rounded_classes = 'edgtf-rounded-tab';
	if ($animate_rounded_tab == 'yes') {
		$rounded_classes .= ' edgtf-animate-rounded-tab';
	}

	if($rounded_tab_color != '') {
		$rounded_tab_style[] = 'fill:'. $rounded_tab_color;
	}

	if ($disable_rounded_tab_on_touch == 'yes'){
		$rounded_classes .= ' edgtf-rounded-tab-disable';
	}

	if($rounded_tab_position == 'top' || $rounded_tab_position == 'top-bottom') {
		$after_wrapper_open .= '<svg class="edgtf-rounded-tab-top '.$rounded_classes.'" ' . fair_edge_get_inline_style($rounded_tab_style) . ' x="0px" y="0px" width="134.983px" height="35.995px" viewBox="0 0 134.983 35.995" xml:space="preserve">';
			$after_wrapper_open .= '<path d="M67.492,0C39.958,0,41.958,35.995,0,35.995c19.162,0,67.492,0,67.492,0s48.33,0,67.491,0	C93.026,35.995,95.026,0,67.492,0z"/>';
		$after_wrapper_open .= '</svg>';
	}

	if($rounded_tab_position == 'bottom' || $rounded_tab_position == 'top-bottom') {
		$before_wrapper_close .= '<svg class="edgtf-rounded-tab-bottom '.$rounded_classes.'" ' . fair_edge_get_inline_style($rounded_tab_style) . ' x="0px" y="0px" width="134.983px" height="35.995px" viewBox="0 0 134.983 35.995" xml:space="preserve">';
			$before_wrapper_close .= '<path d="M134.983,0c-19.161,0-67.49,0-67.49,0S19.162,0,0,0 c41.958,0,39.958,35.996,67.492,35.996C95.026,35.996,93.026,0,134.983,0z"/>';
		$before_wrapper_close .= '</svg>';
	}
...
if($video == 'show_video'){
	...
	if($video_image) {
		$video_output .= '<div class="edgtf-mobile-video-image" ' . fair_edge_get_inline_attr($video_mobile_style, 'style') . ')"></div>';
	}
	$video_output .= '<div ' . fair_edge_get_class_attribute($video_overlay_class) . fair_edge_get_inline_attr($video_overlay_style, 'style') . '></div>';
	$video_output .= '<div class="edgtf-video-wrap">';
	$video_output .= '<video class="edgtf-video" width="1920" height="800" '. fair_edge_get_inline_attr($video_attrs, 'poster')  .' controls="controls" preload="auto" loop autoplay muted>';
	...
	if($v_image) {
		$video_output .= '<img ' . fair_edge_get_inline_attr($v_image, 'src') . ' width="1920" height="800" title="No video playback capabilities" alt="'.esc_html__('video thumb','fair').'" />';
	}
	$video_output .='</object>';
	$video_output .='</video>';
	$video_output .='</div>';
