...
class FairEdgeLike {
	...
	private function __construct() {
		...
		add_action('wp_ajax_fair_edge_like', array( $this, 'ajax'));
		add_action('wp_ajax_nopriv_fair_edge_like', array( $this, 'ajax'));
	}
	...
	public function add_like() {
		...
		$title = esc_html__('Like this', 'fair');
		if( isset($_COOKIE['edgtf-like_'. $post->ID]) ){
			$class = 'edgtf-like liked';
			$title = esc_html__('You already like this!', 'fair');
		}
...
if ( !function_exists( 'fair_edge_create_like' ) ) {

	function fair_edge_create_like() {
		FairEdgeLike::get_instance();
	}

	add_action('after_setup_theme', 'fair_edge_create_like');
}
