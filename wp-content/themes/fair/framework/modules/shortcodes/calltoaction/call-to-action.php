<?php
namespace FairEdge\Modules\Shortcodes\CallToAction;
...
class CallToAction implements ShortcodeInterface {
	...
	public function vcMap() {
		...
		$call_to_action_button_IconCollections = fair_edge_icon_collections()->iconCollections;
		...
		vc_map( array(
				'name' => esc_html__('Call To Action', 'fair'),
				'base' => $this->getBase(),
				'category' => 'by EDGE',
				...
				'params' => array_merge(
					...
					fair_edge_icon_collections()->getVCParamsArray(array('element' => 'type', 'value' => array('with-icon'))),
					array(
						array(
							'type' 			=> 'textfield',
						...
						array(
							...
							'value' => array_merge(array('No Icon' => ''),fair_edge_icon_collections()->getIconCollectionsVC()),
							'save_always' => true,
							'dependency' => array('element' => 'show_button', 'value' => array('yes'))
						)
					),
	...
	public function render($atts, $content = null) {
		...
		$args = array_merge($args, fair_edge_icon_collections()->getShortcodeParams(),$call_to_action_icons_form_fields);
		...
		$html = fair_edge_get_shortcode_module_template_part('templates/call-to-action-template', 'calltoaction', '', $params);
	...
	private function getCallToActionIcon($params) {
		...
		$icon = fair_edge_icon_collections()->getIconCollectionParamNameByKey($params['icon_pack']);
		$iconStyles = array();
		$iconStyles['icon_attributes']['style'] = $this->getIconStyles($params);
		$call_to_action_icon = '';
		if(!empty($params[$icon])){			
			$call_to_action_icon = fair_edge_icon_collections()->renderIcon( $params[$icon], $params['icon_pack'], $iconStyles );
		}
	...
	private function getButtonParameters($params) {
		...
		$button_params_array['type'] = 'solid-two';
		...
		if(!empty($params['button_icon_pack'])) {
			...
			$iconPackName = fair_edge_icon_collections()->getIconCollectionParamNameByKey($params['button_icon_pack']);
