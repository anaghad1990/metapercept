...
if(!function_exists('fair_edge_single_portfolio')) {
    function fair_edge_single_portfolio() {
        $portfolio_template = fair_edge_get_portfolio_single_type();
        ...
        if ($portfolio_template == 'gallery') {
            $params['holder_class'][] = 'edgtf-portfolio-gallery-' . fair_edge_options()->getOptionValue('portfolio_single_numb_columns');
        }

        fair_edge_get_module_template_part('templates/single/holder', 'portfolio', '', $params);
    }
}

if(!function_exists('fair_edge_get_portfolio_single_type')) {
    function fair_edge_get_portfolio_single_type() {
        return fair_edge_get_meta_field_intersect('portfolio_single_template');
    }
}

if(!function_exists('fair_edge_get_portfolio_single_media')) {
    function fair_edge_get_portfolio_single_media() {
	    ...
	    $single_type = fair_edge_get_portfolio_single_type();
        if($image_ids !== '') {
		        ...
		        foreach($image_ids as $image_id) {
			        ...
			        switch($size){
				        case 'edgtf-large-height-masonry-item' :
					        $img_size = 'fair_edge_large_height';
					        break;
				        case 'edgtf-large-width-masonry-item' :
					        $img_size = 'fair_edge_large_width';
					        break;
				        case 'edgtf-large-width-height-masonry-item' :
					        $img_size = 'fair_edge_large_width_height';
					        break;
				        default:
					        $img_size = 'fair_edge_square';
					        break;
			        }
        ...
        if(is_array($videos) && count($videos)) {
            usort($videos, 'fair_edge_compare_portfolio_videos');
            foreach($videos as $video) {
                ...
                if(!empty($video['portfoliovideotype'])) {
                    ...
                    $media['video_url']   = fair_edge_portfolio_get_video_url($video);
...
if(!function_exists('fair_edge_portfolio_get_video_url')) {
    function fair_edge_portfolio_get_video_url($video) {
...
if(!function_exists('fair_edge_portfolio_get_media_html')) {
    function fair_edge_portfolio_get_media_html($media) {
        global $wp_filesystem;
        $params = array();

        //For adding image overlay in gallery template type
        $params['gallery'] = (fair_edge_get_portfolio_single_type() == 'gallery') ? true : false;

        if($media['type'] == 'image') {
            $params['lightbox'] = fair_edge_options()->getOptionValue('portfolio_single_lightbox_images') == 'yes';
        ...
        if(in_array($media['type'], array('youtube', 'vimeo'))) {
            $params['lightbox'] = fair_edge_options()->getOptionValue('portfolio_single_lightbox_videos') == 'yes';
        ...
        fair_edge_get_module_template_part('templates/single/media/'.$media['type'], 'portfolio', '', $params);
    }
}

if(!function_exists('fair_edge_compare_portfolio_videos')) {
    /**
     ...
     */
    function fair_edge_compare_portfolio_videos($a, $b) {
...
if(!function_exists('fair_edge_compare_portfolio_options')) {
    /**
     ...
     * @return int result of comparison
     */
    function fair_edge_compare_portfolio_options($a, $b) {
...
if(!function_exists('fair_edge_portfolio_get_info_part')) {
    function fair_edge_portfolio_get_info_part($part) {
        $portfolio_template = fair_edge_get_portfolio_single_type();

        fair_edge_get_module_template_part('templates/single/parts/'.$part, 'portfolio', $portfolio_template, array('type'=>$portfolio_template));
    }
}