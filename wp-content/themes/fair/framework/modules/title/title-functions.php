...
if(!function_exists('fair_edge_get_title')) {
    /**
     * Loads title area HTML
     */
    function fair_edge_get_title() {
        $id = fair_edge_get_page_id();

        extract(fair_edge_title_area_height());
        extract(fair_edge_title_area_background());
        ...
        } elseif(is_singular('post') && fair_edge_get_meta_field_intersect('blog_single_type') == 'with-title-image') {
			$show_title_area = false;
		} else {
            $show_title_area = fair_edge_options()->getOptionValue('show_title_area') == 'yes' ? true : false;
        }
        ...
        if(($meta_temp = get_post_meta($id, "edgtf_title_area_type_meta", true)) !== ""){
            ...
            $type = fair_edge_options()->getOptionValue('title_area_type');
        }

        $hide_title_text = false;
        if (get_post_meta($id,'edgtf_hide_title_text_meta',true) == 'yes'){
        	$hide_title_text = true;
        }
        ...
        if(($meta_temp = get_post_meta($id, "edgtf_title_area_enable_breadcrumbs_meta", true)) !== ""){
            ...
            $enable_breadcrumbs = fair_edge_options()->getOptionValue('title_area_enable_breadcrumbs') == 'yes' ? true : false;
        }
        ...
        $subtitle_position = fair_edge_get_meta_field_intersect('subtitle_position');
        if($subtitle_position == ''){
            $subtitle_position = 'below';
        }

        //check separator and separator parameters        
        $separator_show = fair_edge_get_meta_field_intersect('separator_yesno');
        $separator_params = array();
        if($separator_show == 'yes'){
            $separator_color = fair_edge_get_meta_field_intersect('separator_color');
            if ($separator_color !== ''){
            	$separator_params['color'] = $separator_color;
            }
            $separator_alignment = fair_edge_get_meta_field_intersect('title_area_content_alignment');
            if ($separator_alignment !== ''){
            	$separator_params['position'] = $separator_alignment;
            }
        }

        //check rounded tab visibility and color      
        $rounded_tab = false;
		if (fair_edge_get_meta_field_intersect('title_area_rounded_tab') == 'yes'){
			$rounded_tab = true;
		}

        $rounded_tab_style = '';
        if (($meta_temp = fair_edge_get_meta_field_intersect('title_area_tab_color')) !== ''){
        	$rounded_tab_style = 'fill:'.$meta_temp;
        }

        $animate_rounded_tab = '';
        if (fair_edge_get_meta_field_intersect('title_area_tab_animate') == 'yes'){
            $animate_rounded_tab = 'edgtf-animate-rounded-tab';
        }
        ...
        $parameters = array(
            ...
            'separator_show' => $separator_show,
            'separator_params' => $separator_params,
            'rounded_tab' => $rounded_tab,
            'rounded_tab_style' => $rounded_tab_style,
            'animate_rounded_tab' => $animate_rounded_tab
        );

        $parameters = apply_filters('fair_edge_title_area_height_params', $parameters);

        fair_edge_get_module_template_part('templates/title', 'title', '', $parameters);
    }
}

if(!function_exists('fair_edge_get_title_text')) {
    /**
     * Function that returns current page title text. Defines fair_edge_title_text filter
     ...
     */
    function fair_edge_get_title_text() {
        ...
        if (is_tag()) {
            ...
            $title = single_term_title("", false).esc_html__(' Tag', 'fair');
        }
        ...
        elseif (is_author()) {
            ...
            $title = esc_html__('Author:', 'fair') . " " . get_the_author();
        }
		...
		elseif (is_search()) {
			...
			$title = esc_html__('Search Results for', 'fair').': '.get_search_query();
		}

		//is single post page
        elseif (is_singular('post')) {
			if (fair_edge_options()->getOptionValue('blog_single_title_in_title_area') == "yes") {
				$title = get_the_title($id);
			} else {
				$title = esc_html__('Blog', 'fair');
			}
		}
        ...
        elseif (is_404()) {
            ...
            if(fair_edge_options()->getOptionValue('404_title') != "") {
                //get it from options
                $title = fair_edge_options()->getOptionValue('404_title');
            } else {
                //get default 404 page title
                $title = esc_html__('404 - Page not found', 'fair');
            }
        }

        //is WooCommerce installed and is shop or single product page?
        elseif(fair_edge_is_woocommerce_installed() && (is_shop() || is_singular('product'))) {
        ...
        elseif(fair_edge_is_woocommerce_installed() && (is_product_category() || is_product_tag())) {
            global $wp_query;
        ...
        $title = apply_filters('fair_edge_title_text', $title);

        return $title;
    }
}

if(!function_exists('fair_edge_title_text')) {
    /**
     * Function that echoes title text.
     *
     * @see fair_edge_get_title_text()
     */
    function fair_edge_title_text() {
        echo fair_edge_get_title_text();
    }
}

if(!function_exists('fair_edge_subtitle_text')) {
    /**
     * Function that echoes subtitle text.
     *
     */
    function fair_edge_subtitle_text() {
        $id = fair_edge_get_page_id();
        $subtitle 	= '';
...
if(!function_exists('fair_edge_custom_breadcrumbs')) {
    /**
     ...
     * @see is_front_page()
     * @see is_category()
     * @see fair_edge_is_product_category()
     * @see get_search_query()
     */
    function fair_edge_custom_breadcrumbs() {
        global $post;

        $output = "";
        $homeLink = esc_url(home_url('/'));
        $pageid = fair_edge_get_page_id();
        $bread_style = "";
        ...
        $delimiter = '<span class="edgtf-delimiter" '.fair_edge_get_inline_style($bread_style).'>&nbsp;/&nbsp;</span>'; // delimiter between crumbs
        $home = get_bloginfo('name'); // text for the 'Home' link
        $showCurrent = 1; // 1 - show current post/page title in breadcrumbs, 0 - don't show
        $before = '<span class="edgtf-current" '.fair_edge_get_inline_style($bread_style).'>'; // tag before the current crumb
        $after = '</span>'; // tag after the current crumb

        if (is_home() && !is_front_page()) {
            $output = '<div class="edgtf-breadcrumbs"><div class="edgtf-breadcrumbs-inner"><a '.fair_edge_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a>' . $delimiter . ' <a '.fair_edge_get_inline_style($bread_style).' href="' . $homeLink . '">'. get_the_title($pageid) .'</a></div></div>';
        ...
        elseif(is_front_page()) {
            if ($showOnHome == 1) $output = '<div class="edgtf-breadcrumbs"><div class="edgtf-breadcrumbs-inner"><a '.fair_edge_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a></div></div>';
        }

        else {

            $output .= '<div class="edgtf-breadcrumbs"><div class="edgtf-breadcrumbs-inner"><a '.fair_edge_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a>' . $delimiter;

            if ( is_category() || fair_edge_is_product_category()) {
                ...
                $output .= $before . esc_html__('Search results for ', 'fair') . '"' . get_search_query() . '"' . $after;

            } elseif ( is_day() ) {
                $output .= '<a '.fair_edge_get_inline_style($bread_style).' href="' . get_year_link(get_the_time('Y')) . '">' . get_the_time('Y') . '</a>' . $delimiter;
                $output .= '<a '.fair_edge_get_inline_style($bread_style).' href="' . get_month_link(get_the_time('Y'),get_the_time('m')) . '">' . get_the_time('F') . '</a>' . $delimiter;
                ...
                $output .= '<a '.fair_edge_get_inline_style($bread_style).' href="' . get_permalink($parent) . '">' . $parent->post_title . '</a>';
                ...
                while ($parent_id) {
                    ...
                    $breadcrumbs[] = '<a '.fair_edge_get_inline_style($bread_style).' href="' . get_permalink($page->ID) . '">' . get_the_title($page->ID) . '</a>';
                ...
                $output .= $before . esc_html__('Posts tagged ','fair') .'"' . single_tag_title('', false) . '"' . $after;

            } elseif ( is_author() ) {
                global $authordata;
                $output .= $before . esc_html__('Articles posted by ','fair') . $authordata->display_name . $after;
            ...
            if ( get_query_var('paged') ) {
                ...
                $output .= $before . "&nbsp;(" . esc_html__('Page', 'fair') . ' ' . get_query_var('paged') . ")" . $after;
...
if(!function_exists('fair_edge_get_title_area_height')) {

    /**
     * Function that returns title height
     **/
    function fair_edge_get_title_area_height() {
        ...
        if(($meta_temp = get_post_meta($id, 'edgtf_title_area_type_meta', true)) != '') {
            ...
            $title_type = fair_edge_options()->getOptionValue('title_area_type');
        }
        ...
        } elseif(fair_edge_options()->getOptionValue('title_area_height') !== '') {
            $title_height = fair_edge_options()->getOptionValue('title_area_height');
        }

        return apply_filters('fair_edge_title_area_height', $title_height);
    }
}

if(!function_exists('fair_edge_get_title_content_padding')) {
    /**
     * Function that returns title content pading
     **/

    function fair_edge_get_title_content_padding() {
        return apply_filters('fair_edge_title_content_padding', 0);
    }
}


if(!function_exists('fair_edge_title_area_height')) {
    /**
     * Function that returns title height and padding to be applied in template
     **/

    function fair_edge_title_area_height() {
        $id = fair_edge_get_page_id();
        $title_height_and_padding = array();
        $title_height          = fair_edge_get_title_area_height();
        $header_height_padding = fair_edge_get_title_content_padding();
        ...
        if(($meta_temp = get_post_meta($id, "edgtf_title_area_background_image_responsive_meta", true)) != "") {
            ...
            $is_img_responsive = fair_edge_options()->getOptionValue('title_area_background_image_responsive');
        }

        if(($meta_temp = get_post_meta($id, "edgtf_title_area_vertial_alignment_meta", true)) !== '') {
            $title_vertical_alignment = $meta_temp;
        }else{
            $title_vertical_alignment = fair_edge_options()->getOptionValue('title_area_vertial_alignment');;
        }
...
if(!function_exists('fair_edge_title_area_background')) {
    /**
     * Function that returns title background style be applied in template
     **/

    function fair_edge_title_area_background() {
        $id = fair_edge_get_page_id();
        $show_title_img = true;
        ...
        if(($meta_temp = get_post_meta($id, "edgtf_title_area_background_color_meta", true)) != ""){
            ...
            $background_color = fair_edge_options()->getOptionValue('title_area_background_color');
        }
        ...
        if(($meta_temp = get_post_meta($id, "edgtf_title_area_background_image_meta", true)) != ""){
            ...
            $background_image = fair_edge_options()->getOptionValue('title_area_background_image');
        }

        //check for background image width
        $background_image_width = "";
        if($background_image !== ''){
            $background_image_width_dimensions_array = fair_edge_get_image_dimensions($background_image);
