...
?><article id="post-<?php the_ID(); ?>" <?php post_class( is_page()?'':'blog_post' ); ?>>
		...
		<div class="entry-content blog_postcontent">
			...
			<?php 
				...
				if( $devn->cfg['excerptImage'] == 1 && !is_page() && !is_single() )
				{
				...
				if( $devn->cfg['excerptImage'] == 1 && is_single() ){
					
					$img = $devn->get_featured_image( $post, false );
				...
				<?php if( !is_page() ): ?>
					...
					<header class="entry-header animated ext-fadeInUp">
						...
						<?php if( $devn->cfg['showDateMeta']==1 && !is_page() ): ?>
						...
						<h3 class="entry-title">
							<a href="<?php the_permalink(); ?>" title="<?php printf( esc_attr__( 'Permalink to %s', 'aaikadomain' ), the_title_attribute( 'echo=0' ) ); ?>" rel="bookmark">
								<?php the_title(); ?>
							</a>
						...
						if ( 'post' == get_post_type() ){
							...
							if ( $devn->cfg['showMeta'] ==  1 ){ 
								devn::posted_on( 'post_meta_links ' );
							}
				...
				if( ( get_option('rss_use_excerpt') == 1 || is_search() ) && !is_single() && !is_page() ){
					...
					the_excerpt();
					echo '<a href="'.get_the_permalink().'">'.__('read more &#187;','aaikadomain').'</a>';
					
				}else{
					the_content( __( 'Read more &#187;', 'aaikadomain' ) ); 				
				}
				
				wp_link_pages( array( 'before' => '<div class="page-link"><span>' . __( 'Pages:', 'aaikadomain' ) . '</span>', 'after' => '</div>' ) ); 
