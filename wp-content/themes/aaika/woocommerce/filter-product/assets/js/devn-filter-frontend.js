...
jQuery(function($){
    ...
    //wrap the container
    $(devn_filter.container).wrap('<div class="dev-filter-container"></div>');
    $('.woocommerce-info').wrap('<div class="dev-filter-container"></div>');

   $(document).on('click', '.devn-filter a', function(e){
       ...
       if( $(this).data('type') == 'select' ) {
           ...
           $(this).parents('div.devn-filter-ajax-navigation').find('a.dev-filter-select-open').removeClass('active');

           $(this).parent().find('div.devn-filter-select-wrapper').animate({
        ...
        //loading
        $(devn_filter.container).html('').addClass('devn-filter-loading');
        $(devn_filter.pagination).hide();
        $(devn_filter.result_count).hide();

        $.ajax({
            url: href,
            success: function( response ){
                $(devn_filter.container).removeClass('devn-filter-loading');

                //container
                if( $(response).find(devn_filter.container).length > 0 ) {
                    $('.dev-filter-container').html( $(response).find(devn_filter.container) );
                } else {
                    $('.dev-filter-container').html( $(response).find('.woocommerce-info') );
                }


                //pagination
                if( $(response).find(devn_filter.pagination).length > 0 ) {
                    //se non esiste lo creo
                    if( $(devn_filter.pagination).length == 0 ) {
                        $.jseldom( devn_filter.pagination ).insertAfter( $(devn_filter.container) );
                    }

                    $(devn_filter.pagination)
                        .html( $(response).find(devn_filter.pagination).html())
                        .show();
                }

                //result count
                if( $(response).find(devn_filter.result_count).length > 0 ) {
                    $(devn_filter.result_count).html( $(response).find(devn_filter.result_count).html()).show();
                }


                //load new widgets
                $('.devn-filter-ajax-navigation').each(function(){
                    var id = $(this).attr('id');
                ...
                $(document).trigger("devn-filter-ajax-filtered");
            }
    ...
    function dev_open_select_dropdown(element){

        $(element).parent().find('div.devn-filter-select-wrapper').css("z-index", "1").animate({
        ...
        $(element).parent().find('a.dev-filter-select-open').addClass('active');
    }

    function dev_close_select_dropdown(element){
        ...
        $(element).parent().find('a.dev-filter-select-open').removeClass('active');
    }

    var dev_hidden_filters_wrapper = function () {

        $('div.devn-filter-select-wrapper').animate({
        ...
        $('a.dev-filter-select-open').removeClass('active');
    }

    var dev_active_filter = function() {

        var filter_number = $('div.devn-filter-select-wrapper ul.devn-filter-select li.chosen').length;

        dev_hidden_filters_wrapper();

        $('div.devn-filter-select-wrapper').each(function() {

            var filter_name="";
            var chosen =  $(this).find('ul.devn-filter-select li.chosen').each(function(){
            ...
            if(filter_name != "") {
                $(this).parent().find('a.dev-filter-select-open').text(filter_name);
            }
       })
    }

    $(document).on('click' , 'a.dev-filter-select-open.active' , function(e) {
        e.preventDefault();
        dev_close_select_dropdown(this);
    });

    $(document).on('click' , 'a.dev-filter-select-open:not(.active)' , function(e) {
        e.preventDefault();
        dev_open_select_dropdown(this);
    });

    $(document).on('ready devn-filter-ajax-filtered', dev_active_filter);

    $(document).on('ready', dev_hidden_filters_wrapper );

    $('body').on('click', function(e){

        if( !$(e.target).hasClass('dev-filter-select-open') ) {
            dev_hidden_filters_wrapper();
        }
