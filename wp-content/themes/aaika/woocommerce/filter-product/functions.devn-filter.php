 ...
 */
function devn_filter_dropdown_attributes( $selected, $echo = true ) {
    $attributes = DEVN_FILTER_Helper::attribute_taxonomies();
    $options    = "";
 ...
 * @return string
 */
function devn_filter_attributes_table( $type, $attribute, $id, $name, $values = array(), $echo = true ) {
    $return = '';
    ...
    elseif ( 'color' == $type ) {
        if ( ! empty( $terms ) ) {
            $return = sprintf( '<table><tr><th>%s</th><th>%s</th></tr>', __( 'Term', 'aaikadomain' ), __( 'Color', 'aaikadomain' ) );

            foreach ( $terms as $term ) {
                $return .= "<tr><td><label for='{$id}{$term->term_id}'>{$term->name}</label></td><td><input type='text' id='{$id}{$term->term_id}' name='{$name}[colors][{$term->term_id}]' value='" . ( isset( $values[$term->term_id] ) ? $values[$term->term_id] : '' ) . "' size='3' class='devn-colorpicker' /></td></tr>";
            }
    ...
    elseif ( 'label' == $type ) {
        if ( ! empty( $terms ) ) {
            $return = sprintf( '<table><tr><th>%s</th><th>%s</th></tr>', __( 'Term', 'aaikadomain' ), __( 'Labels', 'aaikadomain' ) );
 ...
 * @return bool
 */
function devn_filter_displayed() {
    ...
    if ( is_active_widget( false, false, 'devn-filter-ajax-navigation', true ) ) {
        return true;
    }
    else {
        return false;
...
if ( ! function_exists( 'devn_curPageURL' ) ) {
    /**
     * Retrieve the current complete url
     */
    function devn_curPageURL() {
        $pageURL = 'http';
...
if ( ! function_exists( 'devn_get_terms' ) ) {
    /**
     ...
     */
    function devn_get_terms( $case, $taxonomy ) {

        $exclude = apply_filters( 'devn_filter_exclude_terms', array() );
...
if ( ! function_exists( 'devn_term_is_child' ) ) {
    /**
     * Return true if the term is a child, false otherwise
     *
     * @param $term The term object
     */
    function devn_term_is_child( $term ) {
...
if ( ! function_exists( 'devn_term_has_child' ) ) {
    /**
     ...
     */
    function devn_term_has_child( $term, $taxonomy ) {
        global $woocommerce;
