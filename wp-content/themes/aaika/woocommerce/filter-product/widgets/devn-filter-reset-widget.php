...
if ( ! class_exists( 'DEVN_FILTER' ) ) {
    ...
    class DEVN_RESET_FILTER_Widget extends WP_Widget {

        function __construct() {
            $widget_ops  = array( 'classname' => 'devn-filter-ajax-reset-navigation devn-filter-ajax-navigation woocommerce widget_layered_nav', 'description' => __( 'Reset all filters setted by DEVN Filter Widget', 'aaikadomain' ) );
            $control_ops = array( 'width' => 400, 'height' => 350 );
            parent::__construct( 'devn-filter-ajax-reset-navigation', __( 'DEVN Woo Reset Filter Widget', 'aaikadomain' ), $widget_ops, $control_ops );
        }
        ...
        function widget( $args, $instance ) {
            ...
            if ( count( $_chosen_attributes ) > 0 || $min_price > 0 || $max_price > 0 ) {
                ...
                $label = isset( $instance['label'] ) ? apply_filters( 'devn-filter-reset-navigation-label', $instance['label'], $instance, $this->id_base ) : '';

                //clean the url
                $link = devn_curPageURL();
                ...
                echo "<div class='devn-filter'><a class='devn-filter-reset-navigation button' href='{$link}'>" .$label. "</a></div>";
                print( $after_widget );
                echo ob_get_clean();
            }
            else {
        ...
        function form( $instance ) {
            ...
            $defaults = array(
                'title' => '',
                'label' => __( 'Reset All Filters', 'aaikadomain' )
            );
            ...
            <p>
                <label>
                    <strong><?php _e( 'Button Label', 'aaikadomain' ) ?>:</strong><br />
