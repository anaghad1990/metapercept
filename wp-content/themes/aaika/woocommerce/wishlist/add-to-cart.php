...
global $woocommerce, $devn_wishlist, $devn;
...
$error_link_url = $devn_wishlist->get_wishlist_url();
...
if( isset( $_GET['redirect_to_cart'] ) && $_GET['redirect_to_cart'] == 'true' )
    ...
    { $redirect_url = $devn_wishlist->get_wishlist_url(); }

//get the details of the product
$details = $devn_wishlist->get_product_details( $_GET['wishlist_item_id'] );
...
if( $woocommerce->cart->add_to_cart( $details[0]['prod_id'], 1 ) ) {
    ...
    if( $devn->cfg['wl_remove'] == 1 )
        { $devn_wishlist->remove( $details[0]['ID'] ); }
