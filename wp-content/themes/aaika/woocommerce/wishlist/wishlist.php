...
if( !class_exists( 'DEVN_WISHLIST' ) ) {    
    ...
    class DEVN_WISHLIST {
        ...
        public function __construct( $details ) {              
            ...
            $devn_wishlist_init = new DEVN_WISHLIST_INIT();
        ...
        public function is_product_in_wishlist( $product_id ) {
    		...
    		if( is_user_logged_in() ) {		
    			...
    			$sql = "SELECT COUNT(*) as `cnt` FROM `" . DEVN_WISHLIST_TABLE . "` WHERE `prod_id` = " . intval( $product_id ) . " AND `user_id` = " . intval( $this->details['user_id'] );
    			
    			$results = $wpdb->get_results( $sql );
    			$exists = $results[0]->cnt > 0 ? true : false;
    		} else {
                if( devn_usecookies() ) {
                    $tmp_products = devn_getcookie( 'devn_wishlist_products' );
                } elseif( isset( $_SESSION['devn_wishlist_products'] ) ) {
                    $tmp_products = $_SESSION['devn_wishlist_products'];
                } else {
                    $tmp_products = array();
                }
        ...
        public function add() {
            ...
            if ( isset( $this->details['add_to_wishlist'] ) && is_numeric( $this->details['add_to_wishlist'] ) ) {
                ...
                if( is_user_logged_in() ) {
                    $sql = "INSERT INTO `" . DEVN_WISHLIST_TABLE . "` ( `prod_id`, `quantity`, `user_id`, `dateadded` ) VALUES ( " . $this->details['add_to_wishlist'] . " , $quantity, " . $this->details['user_id'] . ", now() )";
                    ...
                    $cookie = $cookie + devn_getcookie( 'devn_wishlist_products' );
                    
                    devn_setcookie( 'devn_wishlist_products', $cookie );
                    $return = true;  
                } else {
                    $_SESSION['devn_wishlist_products'][$this->details['add_to_wishlist']]['add-to-wishlist'] = $this->details['add_to_wishlist'];
				    $_SESSION['devn_wishlist_products'][$this->details['add_to_wishlist']]['quantity'] = $quantity;
				    $return = true; 
                }
                ...
                if( $return ) {
                    ...
                    $this->errors[] = __( 'Error occurred while adding product to wishlist.', 'aaikadomain' );
                    return "error";
                }
            } 
        }
        ...
        public function remove( $id ) {
            ...
            if( is_user_logged_in() ) {
                $sql = "DELETE FROM `" . DEVN_WISHLIST_TABLE . "` WHERE `ID` = " . $id . " AND `user_id` = " . $this->details['user_id'];
                
                if( $wpdb->query( $sql ) ) {
                    return true;
                } else {
                    $this->errors[] = __( 'Error occurred while removing product from wishlist', 'aaikadomain' );
                    return false;
                }
            } elseif( devn_usecookies() ) {
                ...
                unset( $_SESSION['devn_wishlist_products'][$id] );
			    return true;
            }
        }
        ...
        public function count_products() {
            ...
            if( is_user_logged_in() ) {
                $sql = "SELECT COUNT(*) as `cnt` FROM `" . DEVN_WISHLIST_TABLE . "` WHERE `user_id` = " . get_current_user_id();
                $results = $wpdb->get_results( $sql );
                ...
                if( isset( $_SESSION['devn_wishlist_products'] ) ) 
        			{ return count( $_SESSION['devn_wishlist_products'] ); }
            }
        ...
        public function get_product_details( $id ) {
            ...
            if( is_user_logged_in() ) {
                return $wpdb->get_results( $wpdb->prepare( 'SELECT * FROM `' . DEVN_WISHLIST_TABLE . '` WHERE `prod_id` = %d', $id ), ARRAY_A );
            } elseif( devn_usecookies() ) {
                $cookie = devn_getcookie( 'devn_wishlist_products' );
                ...
                $temp_arr[0] = $_SESSION['devn_wishlist_products'][$id];
    			$temp_arr[0]['prod_id'] = $id;
    			return $temp_arr;
            }
        ...
        public function get_wishlist_url() {
            return get_permalink( get_option( 'devn_wcwl_wishlist_page_id' ) );
        }
        ...
        public function get_nonce_url( $action, $url = '' ) {
            return add_query_arg( '_n', wp_create_nonce( 'devn-wishlist-' . $action ), $url );
        }             
        ...
        public function remove_from_wishlist_ajax() {   
            ...
            if( $this->remove( $_GET['wishlist_item_id'] ) )
                { echo apply_filters( 'devn_wishlist_product_removed_text', __( 'Product successfully removed.', 'aaikadomain' ) ); }
            else {
                echo '#' . $count . '#';
