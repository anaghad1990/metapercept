...
if( !class_exists( 'DEVN_Compare_Frontend' ) ) {


    class DEVN_Compare_Frontend {
         ...
         */
        public $cookie_name = 'devn_woocompare_list';

        /**
         * The action used to view the table comparison
         */
        public $action_view = 'devn-woocompare-view-table';

        /**
         * The action used to add the product to compare list

         */
        public $action_add = 'devn-woocompare-add-product';

        /**
         * The action used to add the product to compare list
         */
        public $action_remove = 'devn-woocompare-remove-product';
        ...
        public function __construct() {
            ...
            // add image size
			add_action( 'init', array( $this, 'devn_set_image_size' ), 36 );
            ...
            // add the shortcode
            $devn->ext['asc']( 'devn_compare_button', array( $this, 'compare_button_sc' ) );
        ...
        public function enqueue_scripts() {
            ...
            // scripts
            wp_enqueue_script( 'devn-woocompare-main', DEVN_WOOCOMPARE_URL . 'assets/js/woocompare.js', array('jquery'), '1.0.0', true );
            wp_localize_script( 'devn-woocompare-main', 'devn_woocompare', array(
                ...
                'added_label' => __( 'Added', 'aaikadomain' ),
                'table_title' => __( 'Product Comparison', 'aaikadomain' ),
                'auto_open' => 'yes'
            ));

            // colorbox
            wp_enqueue_style( 'jquery-colorbox', DEVN_WOOCOMPARE_URL . 'assets/css/colorbox.css' );
            wp_enqueue_script( 'jquery-colorbox', DEVN_WOOCOMPARE_URL . 'assets/js/jquery.colorbox-min.js', array('jquery'), '1.4.21', true );

            // widget
            if ( is_active_widget( false, false, 'devn-woocompare-widget', true ) && ! is_admin() ) {
                wp_enqueue_style( 'devn-woocompare-widget', DEVN_WOOCOMPARE_URL . 'assets/css/widget.css' );
            }
        }
        ...
        public function fields() {
			 ...
			 */
			global $devn;
			if($devn->cfg['woo_comp_p_attribute'] == 1){
		...
		*/
		public function get_option_standard_field(){
			...
			if($devn->cfg['woo_comp_p_image'] == 0){ 
				unset($option_standard_field['image']);
			}
			if($devn->cfg['woo_comp_p_title'] == 0){ 
				unset($option_standard_field['title']);
			}
			if($devn->cfg['woo_comp_p_price'] == 0){ 
				unset($option_standard_field['price']);
			}
			if($devn->cfg['woo_comp_p_addtc'] == 0){ 
				unset($option_standard_field['add-to-cart']);
			}
			if($devn->cfg['woo_comp_p_des'] == 0){ 
				unset($option_standard_field['description']);
			}
			if($devn->cfg['woo_comp_p_stock'] == 0){ 
                    ...
                    switch( $field ) {
                        ...
                        case 'stock':
                            ...
                            if ( empty( $availability['availability'] ) ) {
                                $availability['availability'] = __( 'In stock', 'aaikadomain' );
                            }
                        ...
                        default:
                            ...
                            if ( taxonomy_exists( $field ) ) {
                                ...
                                do_action_ref_array( 'devn_woocompare_field_' . $field, array( $product, &$product->fields ) );
                            }
                            break;
                    }
                }
		...
		public function add_button_follow_position(){
			...
			*/
            ...
            if(!empty( $devn->cfg['woo_comp_single'] )) if ( $devn->cfg['woo_comp_single'] == 1 ){
            	 add_action( 'woocommerce_single_product_summary', array( $this, 'add_compare_link' ), 35 );
            }	 
		}
		
		
		 public function devn_set_image_size() {
		 
			/*
			* Add config image size
			*/
			global $devn;
			$widths = isset($devn->cfg['woo_comp_image_width']) ? $devn->cfg['woo_comp_image_width'] : 220 ;
			$heights = isset($devn->cfg['woo_comp_image_height']) ? $devn->cfg['woo_comp_image_height'] : 154 ;
			$crops = isset($devn->cfg['woo_comp_i_crop']) ? $devn->cfg['woo_comp_i_crop'] : true ;
			add_image_size( 'devn-woocompare-image', $widths, $heights, $crops );	
        ...
        public function standard_fields() {
            return array(
                'image' => __('Image', 'aaikadomain' ),
                'title' => __('Title', 'aaikadomain' ),
                'price' => __('Price', 'aaikadomain' ),
                'add-to-cart' => __('Add to cart', 'aaikadomain' ),
                'description' => __('Description', 'aaikadomain' ),
                'stock' => __( 'Availability', 'aaikadomain' )
            );
        }
        ...
        */
        public function add_compare_link( $product_id = false, $args = array() ) {
            ...
            $is_button = !isset( $button_or_link ) || !$button_or_link ? $devn->cfg['woo_comp_button'] : $button_or_link;
            $button_text = isset($devn->cfg['woo_comp_button_label']) ? $devn->cfg['woo_comp_button_label'] : 'compare';
        ...
        public function stylesheet_url() {
            ...
            $plugin_path   = array( 'path' => plugin_dir_path(__FILE__) . 'assets/css/compare.css', 'url' => DEVN_WOOCOMPARE_URL . 'assets/css/compare.css' );
        ...
        public function list_products_html( $lang = false ) {
            ...
            if ( empty( $this->products_list ) ) {
                echo '<li>' . __( 'No products to compare', 'aaikadomain' ) . '</li>';
                return ob_get_clean();
            }
            ...
            foreach ( $this->products_list as $product_id ) {
                ...
                <li>
                    ...
                    <a href="<?php echo esc_attr( $this->remove_product_url( $product_id ) ); ?>" data-product_id="<?php echo esc_attr( $product_id ); ?>" class="remove" title="<?php _e( 'Remove', 'aaikadomain' ) ?>">x</a>
                </li>
            <?php
            }
