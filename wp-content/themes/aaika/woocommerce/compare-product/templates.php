...
wp_enqueue_script( 'jquery-fixedheadertable', DEVN_WOOCOMPARE_URL . 'assets/js/jquery.dataTables.min.js', array('jquery'), '1.3', true );
wp_enqueue_script( 'jquery-fixedcolumns', DEVN_WOOCOMPARE_URL . 'assets/js/FixedColumns.min.js', array('jquery', 'jquery-fixedheadertable'), '1.3', true );
...
<head>
    ...
    <?php wp_title( _e( 'Product Comparison', 'aaikadomain' ) ); ?></title>
    <link rel="profile" href="http://gmpg.org/xfn/11" />

    <link rel="stylesheet" href="http://fonts.googleapis.com/css?family=Open+Sans:300,400,600,700,800" />
    <link rel="stylesheet" href="<?php echo esc_url( $this->stylesheet_url() ) ?>" type="text/css" />
    <link rel="stylesheet" href="<?php echo DEVN_WOOCOMPARE_URL ?>assets/css/colorbox.css"/>
    <link rel="stylesheet" href="<?php echo DEVN_WOOCOMPARE_URL ?>assets/css/jquery.dataTables.css"/>

    <?php wp_head() ?>

    <style type="text/css">
        body.loading {
            background: url("<?php echo DEVN_WOOCOMPARE_URL ?>assets/images/colorbox/loading.gif") no-repeat scroll center center transparent;
        }
    </style>
</head>
...
<h1>
    ...
    <?php 
		$table_text = $devn->cfg['woo_comp_table_title'];
		echo esc_html( $table_text ); 
	?>
    <?php if ( ! $is_iframe ) : ?><a class="close" href="#"><?php _e( 'Close window [X]', 'aaikadomain' ) ?></a><?php endif; ?>
</h1>
...
<table class="compare-list" cellpadding="0" cellspacing="0"<?php if ( empty( $products ) ) echo ' style="width:100%"' ?>>
    ...
    <?php if ( empty( $products ) ) : ?>
        ...
        <tr class="no-products">
            <td><?php _e( 'No products added in the compare table.', 'aaikadomain' ) ?></td>
        </tr>
        ...
        <tr class="remove">
            ...
            <?php foreach( $products as $i => $product ) : $product_class = ( $i % 2 == 0 ? 'odd' : 'even' ) . ' product_' . $product->id ?>
                <td class="<?php echo esc_attr( $product_class ); ?>">
                    <a href="<?php echo add_query_arg( 'redirect', 'view', $this->remove_product_url( $product->id ) ) ?>" data-product_id="<?php echo esc_attr( $product->id ); ?>"><?php _e( 'Remove', 'aaikadomain' ) ?> <span class="remove">x</span></a>
                </td>
            <?php endforeach ?>
        </tr>
                ...
                <?php foreach( $products as $i => $product ) : $product_class = ( $i % 2 == 0 ? 'odd' : 'even' ) . ' product_' . $product->id ?>
                    <td class="<?php echo esc_attr( $product_class ); ?>"><?php
                        switch( $field ) {
                            ...
                            case 'image':
                                echo '<div class="image-wrap">' . wp_get_attachment_image( $product->fields[$field], 'devn-woocompare-image' ) . '</div>';
                                break;
...
<script type="text/javascript">
    ...
    jQuery(document).ready(function($){
        ...
        var oTable;
        $('body').on( 'devn_woocompare_render_table', function(){
            if( $( window ).width() > 767 ) {
        ...
        }).trigger('devn_woocompare_render_table');
        ...
        $(window).on( 'devn_woocompare_product_removed', function(){
            if( $( window ).width() > 767 ) {
                oTable.fnDestroy(true);
            }
            $('body').trigger('devn_woocompare_render_table');
        });
