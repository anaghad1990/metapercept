! function () {
	tinymce.create('tinymce.plugins.shortcodesultimate', {
		...
		_html2shortcodes: function (content) {
			...
			content = content.replace(/<div.*?class=\"devn-row (.*?)\".*?>(.*?)<\/div>/gi, '[row class="$1"]$2[/column]');
			content = content.replace(/<div.*?class=\"devn-column devn-column-size-(.*?) (.*?)\".*?>(.*?)<\/div>/gi, '[column size="$1" class="$2"]$3[/column]');
			return content;
			// function t(t, n) {
			// e = e.replace(t, n);
			// }
		...
		_shortcodes2html: function (content) {
			...
			return content;
			// return e = tinymce.trim(e),
			// t(/\[column.*?size="(.*?)".*?class="(.*?)".*?\](.*?)\[\/column\]/gi, '<div class="devn-column devn-column-size-$1 $2">$3</div>'),
			// t(/\n/gi, "<br />"),
			// t(/\[b\]/gi, "<strong>"),
			// t(/\[\/b\]/gi, "</strong>"),
			// t(/\[i\]/gi, "<em>"),
		...
		_shortcode2html: function (s) {
			...
			if (s.tag === 'row') {
				...
				return '<div class="devn-row' + cssclass + '">' + s.content + '</div>';
			}
			// Columns
				...
				return '<div class="devn-column devn-column-size-' + size + cssclass + '">' + s.content + '</div>';
			}
		}
	}),
