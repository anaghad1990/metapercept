...
class Su_Tools {
	...
	public static function example() {
		...
		// Cache not found
			...
			ob_start();
			// Prepare data
			$code = $devn->ext['fg']( sanitize_text_field( $_REQUEST['code'] ) );
			// Check for code
			if ( !$code ) die( '<p class="devn-examples-error">' . __( 'Example code does not found, please check it later', 'aaikadomain' ) . '</p>' );
			// Clean-up the code
			...
			foreach ( $chunks as $chunk ) {
				...
				echo '<div class="devn-examples-code"><span class="devn-examples-get-code button"><i class="fa fa-code"></i>&nbsp;&nbsp;' . __( 'Get the code', 'aaikadomain' ) . '</span><textarea rows="' . $rows . '">' . esc_textarea( $chunk ) . '</textarea></div>';
			}
	...
	public static function reset_examples() {
		foreach ( (array) devn_shortcode_data::examples() as $example ) foreach ( (array) $example['items'] as $item ) delete_transient( 'sc/examples/render/' . $item['id'] );
	}
	...
	public static function access() {
		if ( !self::access_check() ) wp_die( __( 'Access denied', 'aaikadomain' ) );
	}
	...
	public static function slide_link_input( $form_fields, $post ) {
		$form_fields['su_slide_link'] = array(
			'label' => __( 'Slide link', 'aaikadomain' ),
			'input' => 'text',
			'value' => get_post_meta( $post->ID, 'su_slide_link', true ),
			'helps' => sprintf( '<strong>%s</strong><br>%s', __( 'www.king-theme.com', 'aaikadomain' ), __( 'Use this field to add custom links to slides used with Slider, Carousel and Custom Gallery components', 'aaikadomain' ) )
		);
