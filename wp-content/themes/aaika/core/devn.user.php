...
class devn_user{
	...
	public function init(){
		wp_register_script('devn-user-script', THEME_URI.'/assets/js/devn.user.js', array('jquery') ); 
    	wp_enqueue_script('devn-user-script');
		
		$url_redirect = home_url().'/wp-admin/profile.php';
		
    	wp_localize_script( 'devn-user-script', 'ajax_user_object', array( 
	        'ajaxurl' => admin_url( 'admin-ajax.php' ),
	        'redirecturl' => $url_redirect,
	        'loadingmessage' => __('Sending user info, please wait...', 'aaikadomain')
	    ));

	    add_action( 'wp_ajax_nopriv_devn_user_login', array($this, 'login') );
	    add_action( 'wp_ajax_nopriv_devn_user_register', array($this, 'register') );
	    add_action( 'wp_ajax_nopriv_devn_user_forgot', array($this, 'forgot_password') );		
	}
			...
			'linstar_addition_fields' => array(
				...
				'fields' => array(
					...
					'country' => array(
						...
						'type'        => 'select',
						'options'     => array( '' => __( 'Select a country&hellip;', 'aaikadomain' ) ) + $this->get_countries()
					)
				)				
			)			
		));
					...
					<tr>
						...
						<td>
							<?php if ( ! empty( $field['type'] ) && 'radio' == $field['type'] ) : ?>
								<?php
									$saved_value = esc_attr( get_user_meta( $user->ID, $devn_key, true ) );									
							...
							<?php elseif ( ! empty( $field['type'] ) && 'select' == $field['type'] ) : ?>
								<select name="<?php echo esc_attr( $key ); ?>" id="<?php echo esc_attr( $key ); ?>" class="<?php echo ( ! empty( $field['class'] ) ? $field['class'] : '' ); ?>" style="width: 25em;">
									<?php
										$selected = esc_attr( get_user_meta( $user->ID, $devn_key, true ) );
										foreach ( $field['options'] as $option_key => $option_value ) : ?>
										<option value="<?php echo esc_attr( $option_key ); ?>" <?php selected( $selected, $option_key, true ); ?>><?php echo esc_attr( $option_value ); ?></option>
									<?php endforeach; ?>
								</select>
							<?php else : ?>
							<input type="text" name="<?php echo esc_attr( $key ); ?>" id="<?php echo esc_attr( $key ); ?>" value="<?php echo esc_attr( get_user_meta( $user->ID, $devn_key, true ) ); ?>" class="<?php echo ( ! empty( $field['class'] ) ? $field['class'] : 'regular-text' ); ?>" />
							<?php endif; ?>
							<br/>
	...
	public function save_addition_fields( $user_id ) {
		...
		foreach( $save_fields as $fieldset ) {
			...
			foreach( $fieldset['fields'] as $key => $field ) {
				...
				if ( isset( $_POST[ $key ] ) ) {
					$devn_key = '_devn_user_'.$key;
					update_user_meta( $user_id, $devn_key, sanitize_text_field( $_POST[ $key ] ) );
				}
			}
		}
	}
	...
	public function get_countries() {
		if ( empty( $this->countries ) ) {
			$this->countries = apply_filters( 'devn_theme_countries', include get_template_directory() . '/core/i18n/countries.php' );
			if ( apply_filters( 'devn_theme_sort_countries', true ) ) {
	...
	public function login(){
	    ...
	    if ( is_wp_error($user_signon) ){
			$output = array(
				'loggedin' => false, 
				'message' => __('Wrong username or password.', 'aaikadomain')
			);			
	    } else {
			$output = array(
				'loggedin' => true, 
				'message' => __('Login successful, redirecting...', 'aaikadomain')
			);			     
	    }
		wp_send_json($output);
	...
	public function register(){
		...
		if ( ! get_option( 'users_can_register' ) ) {
			$output = array(
				'status' => false, 
				'message' => __('Disabled user register.', 'aaikadomain')
			);
			wp_send_json($output);	
			die();
		}
		...
		$agree  	= !empty($_POST['agree'])?$_POST['agree']:0;
		
		if( $agree == 0 ){
			$output = array(
				'loggedin' => false, 
				'message'=>__('You must agree the User Agreement, Terms & Condition.', 'aaikadomain')
			);
			wp_send_json($output);
			die();
		}
		...
		if(!$this->checkEmail($email)){
			$output = array(
				'loggedin' => false, 
				'message' => __('Enter a correct email.', 'aaikadomain')
			);
			wp_send_json($output);	
			die();
		}
		...
		if($password1 != $password2){
			$output = array(
				'loggedin' => false, 
				'message'=>__('Password does not match.', 'aaikadomain')
			);
			wp_send_json($output);				
			die();
		}
		...
		if(strlen($password1) < 6){
			$output = array(
				'loggedin' => false, 
				'message'=>__('Password too short.', 'aaikadomain')
			);
			wp_send_json($output);
			die();
		}
		...
		if ( is_wp_error($user_register) ){
			...
			if(in_array('empty_user_login', $error))
				wp_send_json(
					array(
						'loggedin' => false, 
						'message' => __('Empty user login', 'aaikadomain')
					)
				);
			elseif(in_array('existing_user_login',$error))
				wp_send_json(
					array(
						'loggedin' => false, 
						'message' => __('This username is already registered.', 'aaikadomain')
					)
				);
			elseif(in_array('existing_user_email',$error))
		        wp_send_json(
					array(
						'loggedin' => false, 
						'message' => __('This email address is already registered.', 'aaikadomain')
					)
				);
		} else {
	...
	public function forgot_password(){
		...
		if($this->checkEmail($email)){
			...
			if(email_exists($email)){
				...
				if($request_pw){
					...
					wp_send_json(
						array(
							'status' => true, 
							'message' => __( "Please check your email and reset your password.", 'aaikadomain' )
						)
					);	
				}else{
				...
				/** Sent email **/
				$to = $email;
				$subject = 'Password Reset form devn Theme ['. time() .']' ;
				
				$message = '<p>Hi '. $user->user_login. ', Someone (probably you) has requested a new password for your account on devn Theme <br /></p>';
				...
				wp_send_json(
					array(
						'status'=>false, 
						'message'=>__('This email not exist in my system.', 'aaikadomain')
					)
				);
			}
		}else{
			wp_send_json(
				array(
					'status'=>false, 
					'message'=>__('Enter a validate email.', 'aaikadomain')
				)
			);
		}
