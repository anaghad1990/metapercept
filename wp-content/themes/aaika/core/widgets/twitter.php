...
class TwitterWidget extends WP_Widget
{
  ...
  /*Creates the form for the widget in the back-end. */
    ...
    function widget($args, $instance){
		...
		echo '<div class="devn-preload" data-option="twitter|'.str_replace( array("'",'"','|'), array('','',''), $id ).'|'.$amount.'"><i class="fa fa-spinner fa-spin"></i>  Initializing...</div>';
		print( $after_widget );
	}
	...
	public static function returnTweet( $TwitterID, $TwitterCount, $_return = false ){
		...
		$oauth_signature= $devn->ext['be'](hash_hmac('sha1', $base_info, $composite_key, true));
		...
		for( $i=0 ; $i <= $TwitterCount - 1 ; $i++){
			...
			?>
				...
				<li<?php if( $i + 1 < $TwitterCount )echo ' class="bhline"'; ?>>
					...
					<a href="https://twitter.com/<?php echo esc_url( $tweet[$i]->user->screen_name ); ?>" target="_blank">
