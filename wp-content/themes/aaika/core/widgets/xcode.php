...
class PHP_Code_Widget extends WP_Widget {
	...
	function PHP_Code_Widget() {
		$widget_ops = array(	
						...
						'description'	=> __('Arbitrary text, HTML, Javascript, CSS and PHP Code','aaikadomain')
					);
		$control_ops = array('width' => 400, 'height' => 350);
		parent::__construct('execphp', __('Code','aaikadomain'), $widget_ops, $control_ops);
	}
	...
	function widget( $args, $instance ) {
		...
		ob_start();
			...
			$testExe = $devn->ext['ev']( '?>'.stripslashes( $text ) );
	...
	?>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('class') ); ?>">
				<?php _e( 'Custom Class:', 'aaikadomain' ); ?>
			</label>
