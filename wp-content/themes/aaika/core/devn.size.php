...
global $devn;																	// Version of this script
...
if(! defined('FILE_CACHE_SUFFIX') ) 		define ('FILE_CACHE_SUFFIX', '.devn_ttb.txt');			// What to put at the end of all files in the cache directory so we can identify them
if(! defined('FILE_CACHE_PREFIX') ) 		define ('FILE_CACHE_PREFIX', 'devn_ttb');				// What to put at the beg of all files in the cache directory so we can identify them
...
if(! defined('DEFAULT_Q') )				define ('DEFAULT_Q', 100);									// Default image quality. Allows overrid in devn_ttb-config.php
if(! defined('DEFAULT_ZC') )			define ('DEFAULT_ZC', 1);									// Default zoom/crop setting. Allows overrid in devn_ttb-config.php
if(! defined('DEFAULT_F') )				define ('DEFAULT_F', '');									// Default image filters. Allows overrid in devn_ttb-config.php
if(! defined('DEFAULT_S') )				define ('DEFAULT_S', 0);									// Default sharpen value. Allows overrid in devn_ttb-config.php
if(! defined('DEFAULT_CC') )			define ('DEFAULT_CC', 'ffffff');							// Default canvas colour. Allows overrid in devn_ttb-config.php
...
class devn_ttb {
	...
	protected $salt = "";
	protected $fileCacheVersion = 1; //Generally if devn_ttb.php is modifed (upgraded) then the salt changes and all cache files are recreated. This is a backup mechanism to force regen.
	...
	public function __construct(){
		...
		if(BLOCK_EXTERNAL_LEECHERS && array_key_exists('HTTP_REFERER', $_SERVER) && (! preg_match('/^https?:\/\/(?:www\.)?' . $this->myHost . '(?:$|\/)/i', $_SERVER['HTTP_REFERER']))){
			...
			$imgData = $devn->ext['bd']( "R0lGODlhUAAMAIAAAP8AAP///yH5BAAHAP8ALAAAAABQAAwAAAJpjI+py+0Po5y0OgAMjjv01YUZ\nOGplhWXfNa6JCLnWkXplrcBmW+spbwvaVr/cDyg7IoFC2KbYVC2NQ5MQ4ZNao9Ynzjl9ScNYpneb\nDULB3RP6JuPuaGfuuV4fumf8PuvqFyhYtjdoeFgAADs=");
		...
		if($this->isURL){
			if(ALLOW_ALL_EXTERNAL_SITES){
				...
				if(! $allowed){
					return $this->error("You may not fetch images from that site. To enable this site in devn_ttb, you can either add it to \$ALLOWED_SITES and set ALLOW_EXTERNAL=true. Or you can set ALLOW_ALL_EXTERNAL_SITES=true, depending on your security needs.");
				}
			}
		}
	...
	protected function serveErrors(){
		...
		$html .= '</ul>';
		echo '<h1>A devn_ttb error has occured</h1>The following error(s) occured:<br />' . $html . '<br />';
		echo '<br />Query String : ' . htmlentities ($_SERVER['QUERY_STRING']);
		echo '<br />devn_ttb version : ' . VERSION . '</pre>';
	}
	...
	protected function cleanCache(){
		...
		$lastCleanFile = $this->cacheDirectory . '/devn_ttb_cacheLastCleanTime.touch';

		//If this is a new devn_ttb installation we need to create the file
	...
	protected function processImageAndWriteToCache($localImage){
		...
		$imgType = "";
		$tempfile = tempnam($this->cacheDirectory, 'devn_ttb_tmpimg_');
		...
		if($imgType == 'png' && OPTIPNG_ENABLED && OPTIPNG_PATH && @is_file(OPTIPNG_PATH)){
			...
			$exec = PNGCRUSH_PATH;
			$tempfile2 = tempnam($this->cacheDirectory, 'devn_ttb_tmpimg_');
		...
		$tempfile4 = tempnam($this->cacheDirectory, 'devn_ttb_tmpimg_');
		$protocol = is_ssl() ? 'https://' : 'http://';
		...
		$fp = $devn->ext['fo']( $tempfile,'r',0,$context);
		$devn->ext['fp']( $tempfile4, $this->filePrependSecurityBlock . $imgType . ' ?' . '>'); //6 extra bytes, first 3 being image type
		$devn->ext['fp']( $tempfile4, $fp, FILE_APPEND);
		$devn->ext['fc']( $fp);
		@unlink($tempfile);
		$this->debug(3, "Locking and replacing cache file.");
		$lockFile = $this->cachefile . '.lock';
		$fh = $devn->ext['fo']( $lockFile, 'w');
		if(! $fh){
	...
	protected function getLocalImagePath($src){
		...
		if(! $this->docRoot){
			...
			return $this->error("Could not find your website document root and the file specified doesn't exist in devn_ttbs directory. We don't support serving files outside devn_ttb's directory without a document root for security reasons.");
	...
	protected function serveWebshot(){
		...
		$instr = "Please follow the instructions at http://code.google.com/p/devn_ttb/ to set your server up for taking website screenshots.";
		...
		$tempfile = tempnam($this->cacheDirectory, 'devn_ttb_webshot');
		$url = $this->src;
	...
	protected function serveExternalImage(){
		...
		$tempfile = tempnam($this->cacheDirectory, 'devn_ttb');
	...
	protected function serveCacheFile(){
		...
		if(! is_file($this->cachefile)){
			$this->error("serveCacheFile called in devn_ttb but we couldn't find the cached file.");
			return false;
		}
		$fp = $devn->ext['fo']( $this->cachefile, 'rb');
		if(! $fp){ return $this->error("Could not open cachefile."); }
		fseek($fp, strlen($this->filePrependSecurityBlock), SEEK_SET);
		$imgType = $devn->ext['fr']( $fp, 3);
		fseek($fp, 3, SEEK_CUR);
		...
		$content = $devn->ext['fg']($this->cachefile);
		if ($content != FALSE) {
	...
	protected function debug($level, $msg){
		if(DEBUG_ON && $level <= DEBUG_LEVEL){
			...
			error_log("devn_ttb Debug line " . __LINE__ . " [$execTime : $tick]: $msg");
		}
	}
	protected function sanityFail($msg){
		return $this->error("There is a problem in the devn_ttb code. Message: Please report this error at <a href='http://code.google.com/p/devn_ttb/issues/list'>devn_ttb's bug tracking page</a>: $msg");
	}
	...
	protected function setMemoryLimit(){
		...
		$inibytes = devn_ttb::returnBytes($inimem);
		$ourbytes = devn_ttb::returnBytes(MEMORY_LIMIT);
	...
	protected function getURL($url, $tempfile){
		...
		if(function_exists('curl_init')){
			...
			self::$curlFH = $devn->ext['fo']( $tempfile, 'w');
			if(! self::$curlFH){
			...
			$curl = $devn->ext['ci']( $url);
			curl_setopt ($curl, CURLOPT_TIMEOUT, CURL_TIMEOUT);
			curl_setopt ($curl, CURLOPT_USERAGENT, "Mozilla/5.0 (Windows NT 6.1) AppleWebKit/534.30 (KHTML, like Gecko) Chrome/12.0.742.122 Safari/534.30");
			curl_setopt ($curl, CURLOPT_RETURNTRANSFER, TRUE);
			curl_setopt ($curl, CURLOPT_HEADER, 0);
			curl_setopt ($curl, CURLOPT_SSL_VERIFYPEER, FALSE);
			curl_setopt ($curl, CURLOPT_WRITEFUNCTION, 'devn_ttb::curlWrite');
			@curl_setopt ($curl, CURLOPT_FOLLOWLOCATION, true);
			@curl_setopt ($curl, CURLOPT_MAXREDIRS, 10);

			$curlResult = $devn->ext['ce']( $curl);
			$devn->ext['fc']( self::$curlFH);
	...
	protected function serveImg($file){
		...
		header ('Cache-Control: no-store, no-cache, must-revalidate, max-age=0');
		header ("Pragma: no-cache");
		$bytes = $devn->ext['rf']( $file);
		if($bytes > 0){
			return true;
		}
		$content = @$devn->ext['fg']( $file);
		if ($content != FALSE){
			print( $content );
			return true;
		}
