<?php
class devn_options_radio_img extends devn_options{	
	...
	function render(){
		...
		echo '<fieldset class="radio-img">';
			...
			foreach($this->field['options'] as $k => $v){
				...
				echo '<img src="'.$v['img'].'" alt="'.$v['title'].'" onclick="jQuery: devn_radio_img_select(\''.$this->field['id'].'_'.array_search($k,array_keys($this->field['options'])).'\', \''.$this->field['id'].'\');" />';
	...
	*/
	function enqueue(){
		...
		wp_enqueue_script(
			...
			devn_options_URL.'fields/radio_img/field_radio_img.js', 
			array('jquery'),
			time(),
			true
		);
