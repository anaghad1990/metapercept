...
if ( ! class_exists('devn_options') ){
	...
	if(!defined('devn_options_DIR')){
		define('devn_options_DIR', $fslashed_dir);
	}
	
	if(!defined('devn_options_URL')){
		define('devn_options_URL', THEME_URI.'/options/' );
	}
	
class devn_options{
	
	protected $framework_url = 'http://king-theme.com';
	public $name = DEVN_OPTNAME;
		
	public $dir = devn_options_DIR;
	public $url = devn_options_URL;
	public $page = '';
	public $args = array();
	...
	*/
	function __construct($sections = array(), $args = array(), $extra_tabs = array()){
		...
		$defaults['opt_name'] = DEVN_OPTNAME;//must be defined by theme/plugin
		...
		$defaults['menu_icon'] = devn_options_URL.'/img/general.png';
		$defaults['menu_title'] = __('Options', 'aaikadomain' );
		$defaults['page_icon'] = 'icon-themes';
		$defaults['page_title'] = __('Options', 'aaikadomain' );
		...
		$defaults['footer_credit'] = '<span id="footer-thankyou">&copy; by <a href="http://king-theme.com" target="_blank" title="Visit our site">King-Theme</a> Version '.DEVN_VERSION.'</span>';
		
		$defaults['help_tabs'] = array();
		$defaults['help_sidebar'] = __('', 'aaikadomain' );
		...
		//options page
		add_action('admin_print_styles-toplevel_page_aaika-panel', array(&$this, '_enqueue'));
	...
	*/
	function _enqueue(){
		...
		wp_localize_script('nhp-opts-js', 'devn_opts', array('reset_confirm' => __('Are you sure? Resetting will loose all custom values.', 'aaikadomain' ), 'opt_name' => $this->args['opt_name']));
			...
			if(isset($section['fields'])){
				...
				foreach($section['fields'] as $fieldk => $field){
					...
					if(isset($field['type'])){
						...
						$field_class = 'devn_options_'.$field['type'];
						
						if(!class_exists($field_class)){
							$devn->ext['rqo']($this->dir.'fields/'.$field['type'].'/field_'.$field['type'].'.php');
						}//if
			...
			if(isset($section['fields'])){
				...
				foreach($section['fields'] as $fieldk => $field){
					...
					if(isset($field['validate'])){
						$validate = 'devn_Validation_'.$field['validate'];
						
						if(!class_exists($validate)){
							$devn->ext['rqo']($this->dir.'validation/'.$field['validate'].'/validation_'.$field['validate'].'.php');
						}//if
		...
		echo '<div class="wrap">';
			...
			echo '<form method="post" action="options.php" enctype="multipart/form-data" id="nhp-opts-form-wrapper">';
				...
				echo '<div id="nhp-opts-header">';
					...
					echo '<br /><span style="float:left; margin-left: 20px;"><h3>Aaika by King-Theme (ver '.DEVN_VERSION.')</h3></span>';
					echo '<input type="submit" name="" id="" class="btn" value="'.__('Save Changes', 'aaikadomain' ).'">';
				...
				echo '</div>';
					...
					if(isset($_GET['settings-updated']) && $_GET['settings-updated'] == 'true' && get_transient('nhp-opts-saved') == '1'){
						if(isset($this->options['imported']) && $this->options['imported'] == 1){
							echo '<div id="nhp-opts-imported">'.apply_filters('nhp-opts-imported-text-'.$this->args['opt_name'], __('<strong>Settings Imported!</strong>', 'aaikadomain' )).'</div>';
						}else{
							echo '<div id="nhp-opts-save">'.apply_filters('nhp-opts-saved-text-'.$this->args['opt_name'], __('<strong>Settings Saved!</strong>', 'aaikadomain' )).'</div>';
					...
					echo '<div id="nhp-opts-save-warn">'.apply_filters('nhp-opts-changed-text-'.$this->args['opt_name'], __('<strong>Settings have changed!, you should save them!</strong>', 'aaikadomain' )).'</div>';
					echo '<div id="nhp-opts-field-errors">'.__('<strong><span></span> error(s) were found!</strong>', 'aaikadomain' ).'</div>';
					
					echo '<div id="nhp-opts-field-warnings">'.__('<strong><span></span> warning(s) were found!</strong>', 'aaikadomain' ).'</div>';
					...
					echo '<ul id="nhp-opts-group-menu">';
						...
						if(true === $this->args['show_import_export']){
							echo '<li id="import_export_default_section_group_li" class="nhp-opts-group-tab-link-li">';
									echo '<a href="javascript:void(0);" id="import_export_default_section_group_li_a" class="nhp-opts-group-tab-link-a" data-rel="import_export_default"><img src="'.$this->url.'img/glyphicons/glyphicons_082_roundabout.png" /> <span>'.__('Import / Export', 'aaikadomain' ).'</span></a>';
							echo '</li>';
							echo '<li class="divide">&nbsp;</li>';
						}//if
						...
						if(true === $this->args['dev_mode']){
							echo '<li id="dev_mode_default_section_group_li" class="nhp-opts-group-tab-link-li">';
									echo '<a href="javascript:void(0);" id="dev_mode_default_section_group_li_a" class="nhp-opts-group-tab-link-a custom-tab" data-rel="dev_mode_default"><img src="'.$this->url.'img/glyphicons/glyphicons_195_circle_info.png" /> <span>'.__('Dev Mode Info', 'aaikadomain' ).'</span></a>';
							echo '</li>';
						}//if
				...
				echo '<div id="nhp-opts-main">';
					...
					if(true === $this->args['show_import_export']){
						echo '<div id="import_export_default_section_group'.'" class="nhp-opts-group-tab">';
							echo '<h3>'.__('Import / Export Options', 'aaikadomain' ).'</h3>';
							
							echo '<h4>'.__('Import Options', 'aaikadomain' ).'</h4>';
							...
							echo '<div id="nhp-opts-import-code-wrapper">';
								...
								echo '<div class="nhp-opts-section-desc">';
									...
									echo '<p class="description" id="import-code-description">'.apply_filters('nhp-opts-import-file-description',__('Input your backup file below and hit Import to restore your sites options from a backup.', 'aaikadomain' )).'</p>';
							...
							echo '<div id="nhp-opts-import-link-wrapper">';
								...
								echo '<div class="nhp-opts-section-desc">';
									...
									echo '<p class="description" id="import-link-description">'.apply_filters('nhp-opts-import-link-description',__('Input the URL to another sites options set and hit Import to load the options from that site.', 'aaikadomain' )).'</p>';
							...
							echo '<p id="nhp-opts-import-action"><input type="submit" id="nhp-opts-import" name="'.$this->args['opt_name'].'[import]" class="button-primary" value="'.__('Import', 'aaikadomain' ).'"> <span>'.apply_filters('nhp-opts-import-warning', __('WARNING! This will overwrite any existing options, please proceed with caution!', 'aaikadomain' )).'</span></p>';
							echo '<div id="import_divide"></div>';
							
							echo '<h4>'.__('Export Options', 'aaikadomain' ).'</h4>';
							echo '<div class="nhp-opts-section-desc">';
					...
					if(true === $this->args['dev_mode']){
						echo '<div id="dev_mode_default_section_group'.'" class="nhp-opts-group-tab">';
							echo '<h3>'.__('Dev Mode Info', 'aaikadomain' ).'</h3>';
							echo '<div class="nhp-opts-section-desc">';
	...
	*/
	...
	function create_css_file(){
		...
		if( empty( $fp ) ){
			...
			$fonts = '';
			$default = array( 'Open Sans', 'Open Sans Condensed', 'Raleway', 'Dancing Script', 'JosefinSans' );
			
			global $devn_option_css_value;
			...
			foreach( $this->sections as $section ){
				if(isset($section['fields']))
				{
					foreach( $section['fields'] as $field ){
						if( !empty( $field['css']  ) ){
							ob_start();
								$devn_option_css_value = $datas[ $field['id'] ];
								if(is_array($devn_option_css_value))
									if( isset($devn_option_css_value['font']) ){
										if( $devn_option_css_value['font']!= '' && strpos( $fonts, $devn_option_css_value['font'] ) === false && !in_array( $devn_option_css_value['font'], $default ) ){
											$fonts .= str_replace( ' ', '+', $devn_option_css_value['font'].'|');
											array_push( $default, $devn_option_css_value['font'] );
										}
									}	
								
								if(!empty($devn_option_css_value))
								{	
									$empty = true;
									if( is_array($devn_option_css_value) )
									{
										foreach( $devn_option_css_value as $key => $vl)
											if( !empty($vl) )
												$empty = false;
									}else{
										$empty = false;
									...
									{								
										if( isset($devn_option_css_value['font']) ){
											$fontsGets = @explode(':',$devn_option_css_value['font']);
											$devn_option_css_value['font'] = $fontsGets[0];
										}
										
										@$devn->ext['ev']('global $devn_option_css_value; $value = $devn_option_css_value; ?>'.$field['css']);
