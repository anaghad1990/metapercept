<?php
	...
	*/
	...
	function devn_masonry_assets() {
		wp_enqueue_style('devn-masonry');
		wp_enqueue_script('devn-masonry');
	}
	add_action('wp_print_styles', 'devn_masonry_assets');
			            ...
			            <?php
							...
							if( !empty( $img ) )
							{
								if( strpos( $img , 'youtube') !== false )
								{
									...
									$img = devn_createLinkImage( $img, '570x'.$height );
									
									echo '<img alt="'.get_the_title().'" class="featured-image" src="'.esc_url( $img ).'" />';
							}
